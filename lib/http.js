...
export const safe = (fn) => async (request) => {
  try {
    ...
    // Setup problems (missing database, storage, keys) are shown as they are, so the owner knows what to fix.
    return error(status, status >= 500 && !e.expose ? 'Serverfehler. Bitte später erneut versuchen.' : e.message);
  }
};
