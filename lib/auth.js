...
const secret = () => {
  ...
  if (!s || s.length < 16) throw Object.assign(new Error('SESSION_SECRET fehlt (mind. 16 Zeichen).'), { status: 500 });
  return s;
};
...
export const checkPassword = (password) => {
  ...
  if (!expected) throw Object.assign(new Error('ADMIN_PASSWORD ist nicht gesetzt.'), { status: 500 });
